	...
	-- Git diff viewer
	{
		...
		config = function()
			...
			local function apply_diffview_window_opts(tabpage)
				vim.defer_fn(function()
					...
					for _, winid in ipairs(vim.api.nvim_tabpage_list_wins(tabpage)) do
						local ok, isdiff = pcall(function()
			...
			local function scroll_diff_in_view(cmd)
				...
				for _, winid in ipairs(vim.api.nvim_tabpage_list_wins(tabpage)) do
					local ok2, isdiff = pcall(function()
						return vim.wo[winid].diff
					end)
			...
			vim.api.nvim_create_autocmd("FileType", {
				...
				callback = function(args)
					...
					vim.keymap.set("n", "<C-e>", function()
						scroll_diff_in_view("<C-e>")
					end, { buffer = buf, desc = "Scroll diff panes down" })
					vim.keymap.set("n", "<C-y>", function()
						scroll_diff_in_view("<C-y>")
					end, { buffer = buf, desc = "Scroll diff panes up" })
					-- Scroll by half page
					vim.keymap.set("n", "<C-d>", function()
						scroll_diff_in_view("<C-d>")
					end, { buffer = buf, desc = "Scroll diff panes half-page down" })
					vim.keymap.set("n", "<C-u>", function()
						scroll_diff_in_view("<C-u>")
					end, { buffer = buf, desc = "Scroll diff panes half-page up" })
			...
			vim.api.nvim_create_autocmd("FileType", {
				...
				callback = function()
					...
					vim.keymap.set(
						...
						{ buffer = true, desc = "Quick exit diffview" }
					)
				end,
			})
		end,
	...
	-- Tmux navigation
	{
		...
		config = function()
			vim.keymap.set(
				...
				"<C-l>",
				"<C-\\><C-n><C-w>l",
				{ desc = "Navigate right from Claude terminal", silent = true, remap = false }
			)
		end,
	},
